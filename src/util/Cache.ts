...
export default class Cache {
    ...
    protected async addToLRU(key: string): Promise<any> {
        const lru = await this.getLRU();
    ...
    protected async getLRU(): Promise<string[]> {
    ...
    protected async refreshLRU(key: string): Promise<any> {
        await this.removeFromLRU(key);
        return this.addToLRU(key);
    }

    protected async removeFromLRU(key: string): Promise<void> {
        const lru = await this.getLRU();
    ...
    protected async setLRU(lru: string[]): Promise<void> {
